...
namespace
{
    class CriticalSection
    {
      public:
        CriticalSection()
          : m_previous_posture{ tx_interrupt_control(TX_INT_DISABLE) }
        {
            __DMB();
        }
        ...
        {
            __DMB();
            static_cast<void>(tx_interrupt_control(m_previous_posture));
        }
    ...
    void copy_bytes(volatile void* destination,
...
extern "C" bool runtime_atomic_compare_exchange_8(volatile void*,
                                                    void*,
                                                    std::uint64_t,
                                                    int,
                                                    int)
...
#define RUNTIME_DEFINE_ATOMIC_FETCH_OPERATION(name, expression)                                \
    extern "C" std::uint64_t __atomic_fetch_##name##_8(volatile void* pointer,                 \
                                                        std::uint64_t value,                    \
                                                        int)                                   \
    {                                                                                          \
        return fetch_update(pointer, value, [](std::uint64_t left, std::uint64_t right) {      \
            return expression;                                                                 \
        });                                                                                    \
    }                                                                                          \
    extern "C" std::uint64_t __atomic_##name##_fetch_8(volatile void* pointer,                 \
                                                        std::uint64_t value,                    \
                                                        int)                                   \
    {                                                                                          \
...
extern "C" void runtime_atomic_exchange(std::size_t, void*, void*, void*, int)
  asm("__atomic_exchange");
extern "C" bool runtime_atomic_compare_exchange(std::size_t, void*, void*, void*, int, int)
...
extern "C" void runtime_atomic_exchange(std::size_t size,
                                         void* object,
                                         void* value,
                                         void* result,
                                         int)
